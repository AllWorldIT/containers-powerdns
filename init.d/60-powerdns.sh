...
find /etc/powerdns/conf.d -type f -name "*.conf" -print0 | xargs --no-run-if-empty -0 chmod 0660
...
if [ ! -f /etc/powerdns/conf.d/50-backend-gpgsql.conf ]; then
	...
	if [ -n "$POSTGRES_DATABASE" ]; then
		...
		if [ -z "$POSTGRES_PASSWORD" ]; then
			echo "ERROR: Environment variable 'POSTGRES_PASSWORD' is required"
			exit 1
		fi
		# Output config
...
if [ ! -f /etc/powerdns/conf.d/50-backend-gmysql.conf ]; then
...
if [ -n "$POWERDNS_ENABLE_LUA_RECORDS" -a ! -f /etc/powerdns/conf.d/60-lua-records.conf ]; then
...
if [ -n "$MYSQL_DATABASE" ]; then
	while true; do
		...
		mysqladmin ping --host "$MYSQL_HOST" --user "$MYSQL_USER" --silent --connect-timeout=2 && break || true
		unset MYSQL_PWD
		sleep 1
	done
fi
...
# Setup web access
if [ ! -f /etc/powerdns/conf.d/50-webserver.conf ]; then
